...
#' @param group.theta.multinomial Logical. For use when \code{family = "multinomial"}. When \code{TRUE}, inclusion
#' probabilities are averaged over response/outcome categories, e.g. when \code{iar.prior = FALSE}, even though each
#' category has its own set of parameters associated with its probability of occurrence, there is still only a single
#' global inclusion probability. In an anologous case, but with value \code{FALSE}, each category's parameters have
#' their own inclusion probability. This generalizes to the case when \code{iar.prior = TRUE}.
...
ssnet_fit_beta <- function (x, y, family = c("gaussian", "binomial", "multinomial", "poisson", "cox"),
                       offset = NULL, epsilon = 1e-04, alpha = 0.95,
                       maxit = 50, init = rep(0, ncol(x)), init.theta = 0.5,
                       group.theta.multinomial = TRUE,
                       ...
                       plot.pj = FALSE, im.res = NULL)
{
  ...
  if (family == "multinomial") {
    if (length(init.theta) == 1 & group.theta.multinomial == FALSE) {
      theta <- p <- list()
      for (i in 1:length(unique(y))) {
        theta[[i]] <- p[[i]] <- rep(init.theta, length(gvars))
        names(theta[[i]]) <- names(p[[i]]) <- gvars
      }
    }
  } else {
  ...
  # initialization for each response/outcome category
  if (family == "multinomial") {
    for (i in 1:length(b)) {
  ...
  for (iter in 1:maxit) {
    if (family == "multinomial") {
      ...
      p <- list()
      for (i in 1:length(b)) {
        if (group.theta.multinomial == FALSE) {
          out.i <- update_scale_p(b0 = b[[i]][gvars], ss = ss, theta = theta[[i]], alpha = alpha)
        } else {
          out.i <- update_scale_p(b0 = b[[i]][gvars], ss = ss, theta = theta, alpha = alpha)
        }

        prior.scale[[i]][gvars] <- out.i[[1]]
        if (group.theta.multinomial == TRUE) {
    ...
    if (iar.prior == FALSE) {
      if (family == "multinomial") {
        theta <- list()
        for (i in 1:length(b)) {
          theta[[i]] <- update_ptheta_group2(group.vars = group.vars,
                                             p = p[[i]], p.bound = p.bound)
        }
        if (group.theta.multinomial == TRUE) {
          theta.bind <- t(dplyr::bind_cols(theta))
          theta <- apply(theta.bind, 2, mean)
          names(theta) <- gvars
        }
      ...
      if (family == "multinomial") {
        theta <- list()
        for (i in 1:length(b)) {
          theta[[i]] <- max_q2_iar(iar.data = iar.data, p = p[[i]],
    ...
    if (family == "multinomial" & group.theta.multinomial == FALSE) {
      Pf <- list()
      for (i in 1:length(b)) {
        Pf[[i]] <- 1/(prior.scale[[i]] + 1e-10)
      }
    } else {
