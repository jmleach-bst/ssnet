...
#' @param lambda.criteria Determines the model selection criteria. When
...
#' @note This function is primarily used within
#' \code{\link[ssnet]{compare_ssnet}}, but perhaps could be useful elsewhere.
#' @examples
...
#' @export
glmnet_measure <- function(
    cv.model,
    x = NULL,
    y = NULL,
    lambda.criteria = "lambda.min",
    type.measure = c(
      "default", "mse", "deviance",
      "class", "auc", "mae", "C"
    ),
    family = "gaussian",
    alpha = 1
