...
#' (non-negative counts). For \code{family = "gaussian"}, \code{y} is always been standardized. For \code{family = "binomial"},
...
#' @param epsilon A positive convergence tolerance \code{e}; the iterations converge when \eqn{|dev - dev_old|/(|dev| + 0.1) < e}.
...
#'                              stan_local = TRUE)
ssnet_fit_test <- function (x, y, family = c("gaussian", "binomial", "poisson", "cox"),
                       ...
                       plot.pj = FALSE, im.res = NULL)
{
  ...
  for (iter in 1:maxit) {
    ...
    cat("glmnet successful \n")

    b <- as.numeric(f$beta)
    names(b) <- colnames(x)

    cat("updated b successfully \n")
