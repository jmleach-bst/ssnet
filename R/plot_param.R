...
#'
#' Provides a plot to visualize the probability that each of several models
#' includes specific parameters.
#'
#' @param data A data frame whose first column should be named \code{"model"}
#' and contain model names, and whose remaining columns contain the
#' probabilities that the models included that parameter.
#' @param grp.col A vector of color names that will correspond to each model.
#' @param remove.int Logical. When \code{TRUE} assumes that the second column
#' is for the intercept and removes it. The default is \code{FALSE}.
...
plot_param <- function(
  ...
  prob <- as.vector(mdfp)
  mi <- expand.grid(unique(data$model), seq_len(ncol(mdfp)))
