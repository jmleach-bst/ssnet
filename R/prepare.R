...
#'
#' @param x The design, or input, matrix, of dimension nobs x nvars; each row is an observation vector.
#' @param intercept Logical. When \code{intercept = TRUE}, an intercept is included in the model.
#' @param prior.mean A vector of prior means for the parameters. If \code{length(prior.mean) = 1}, then
#' all parameters are assumed to have a common prior mean.
#' @param prior.sd A vector defining prior standard deviations in the normal priors of the coefficients.
#' If provided, they are starting values of the prior standard deviations for the iterative algorithms.
#' This argument is used within the function \code{bglm()}.If \code{length(prior.sd) = 1}, then
#' all parameters are assumed to have a common prior standard deviation.
#' @param prior.scale A vector containing the prior scale values for double exponential or t prior.
#' In the spike-and-slab lasso model, this is generally only the slab prior scale.
#' @param prior.df A scalar defining the prior degrees of freedom when the t-distribution is used
#' as a prior for the parameters.
#' @param multinomial Logical. When \code{TRUE}, prepare the data for analysis of multinomial outcome
#' according to parameterization in \code{glmnet}, i.e., a unique set of parameters is associated with
#' each outcome category. See section 4 of \insertCite{Friedman:2010}{ssnet} for details.
#' @param outcome.cats Either an integer denoting the number of response categories, or a vector containing
#' outcome categories. Only used when \code{multinomial = TRUE}.
...
#' @return A list containing necessary elements for Bayesian analysis in the package \code{BhGLM}.
#' @note This function is a modified version the function \code{prepare} from the R package \code{BhGLM}.
#' @references
#'
#' \insertRef{Friedman:2010}{ssnet}
prepare <- function (x, intercept, prior.mean, prior.sd,
                     prior.scale, prior.df, group,
                     multinomial = FALSE, outcome.cats = NULL)
{
  x0 <- x
  if (intercept == TRUE) {
    x0 <- x[, -1, drop = FALSE]
  ...
  if (multinomial == TRUE) {
    ...
    return(list(x = x, prior.mean = prior.mean.list, prior.sd = prior.sd.list,
    ...
    return(list(x = x, prior.mean = prior.mean, prior.sd = prior.sd,
                prior.scale = prior.scale, prior.df = prior.df, sd.x = sd.x,
                min.x.sd = min.x.sd, group = group, group.vars = group.vars,
                ungroup.vars = ungroup.vars))
  }
