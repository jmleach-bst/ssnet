...
#' \code{cv.bh3}. Default is \code{FALSE}.
    ...
    for (a in 1:length(alpha)) {
      if (type_error == "training") {
        ...
        glmnet.fit.a <- BhGLM::glmNet(x = x, y = y, family = family,
                                    ...
                                    verbose = verbose)
        glmnet.cv.a <- cv.bh3(glmnet.fit.a, nfolds = nfolds, ncv = ncv,
    ...
    for (i in 1:length(s0)) {
      ...
      for (a in 1:length(alpha)) {
        ...
        if (type_error == "training") {
          ...
          ssiar.cv.i.a <- cv.bh3(ssiar.fit.i.a,
                                 nfolds = nfolds,
                                 ncv = ncv,
                                 classify = classify,
                                 classify.rule = classify.rule,
          ...
          if (ncv == 1) {
            ssiar.mp.i.a <- as.data.frame(t(c(alpha = alpha[a],
                                           s0 = s0[i],
                                           ssiar.cv.i.a$measures)))
          } else {
            ssiar.mp.i.a <- as.data.frame(t(c(alpha = alpha[a],
                                           s0 = s0[i],
                                           ssiar.cv.i.a$measures[1, ])))
          }
        }
        # varaible selection performance
        if (variable_selection == TRUE) {
