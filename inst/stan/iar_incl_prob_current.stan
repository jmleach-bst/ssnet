functions {
  ...
  real icar_normal_lpdf(vector psi, int J, int[] node1, int[] node2, real tau) {
    return -0.5 * tau * dot_self(psi[node1] - psi[node2])
...
data {
  ...
  vector<lower = 0, upper = 1>[J] p; // binary outcomes
}
parameters {
  vector[J] psi; // logit of prior probability of inclusion
  real<lower = 0> sigma; // we sample from sd, sigma, then transform to find precision, tau.
}
transformed parameters{
  vector[J] theta = inv_logit(psi); 
  real <lower = 0> tau = inv(sigma); // See above.
}
model {
